...
echo "----->for instance_nr in $(seq -f "%03g" $upperlimit $lowerlimit);"
#for instance_nr in $((seq -f "%03g" $upperlimit $lowerlimit));
for instance_nr in {234..300};
  do
    ...
    echo ibmcloud is instance-create $MY_INSTANCE_NAME $MY_VPC_ID $MY_ZONE $MY_PROFILE_NAME $MY_VPC_SUBNET_ID --image-id $MY_IMAGE_ID --key-ids $MY_SSH_KEY_ID
